########################
##open up the environment.ts file in the same directory where you find the dockerfile. edit the environment.ts file
...
COPY www/Chatscrum-Angular/ /web/Chatscrum-Angular/
...
RUN git config --global user.name "Jospeh SHOWUNMI"
#RUN cd /web/Chatscrum-Angular/ && sed -i 's/api.chatscrum.com/ciapi.chatscrum.com/g' src/app/data.service.ts;
